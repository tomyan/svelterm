...
**All arcs completed 2026-07-05** — 0.18.0 scroll-region diffing (+ a
border-clip bug fix), 0.19.0 kitty graphics, 0.20.0 DevTools TUI.
...
- [x] Slice 1: detect vertical translation — a diff helper that spots
      when `next` is `prev` shifted by ±N rows over a column range, with
      only the entering rows new.
- [x] Slice 2: emit DECSTBM set-region + index/reverse-index + the new
...
- [x] Slice 3: gate on capability (assume supported; harmless if not —
...
- [x] Slice 1: transmit + place RGBA via the kitty graphics protocol at
      an `<img>`'s cell box (base64 chunked payload, placement id keyed
      to the node).
- [x] Slice 2: lifecycle — delete the placement when the image moves,
      resizes, scrolls out, or unmounts; re-place on change. Keep it
      correct (delete-all-ours + re-place per frame) before clever.
- [x] Slice 3: half-block fallback stays the default; kitty path only
...
- [x] Slice 1: a `svt ui` / `svelterm devtools` command that connects,
...
- [x] Slice 2: selecting a node fetches `CSS.getComputedStyle` +
      `DOM.getBoxModel` and shows them in a side panel (Tabs: Styles /
      Box).
- [x] Slice 3: live refresh (poll `getDocument` or re-fetch on a key);
      README + docs.
